...
void DetectorConstruction::SetupWorldGeometry()
{
 ...
 cout<<"!!!!water name density "<<water_mat->GetName()<<" "<<water_mat->GetDensity()/(g/cm3)<<endl;
 cout<<"!!!!iss name density "<<iss_mat->GetName()<<" "<<iss_mat->GetDensity()/(g/cm3)<<endl;
 cout<<"!!!!LiF name density "<<lif_mat->GetName()<<" "<<lif_mat->GetDensity()/(g/cm3)<<endl;
 cout<<"!!!!composite name density "<<composite_mat->GetName()<<" "<<composite_mat->GetDensity()/(g/cm3)<<endl;
...
/*********************phantom drawing**********************************\
 ...
 rotMatrix2->rotateX(CLHEP::pi/2.);
 rotMatrix2->rotateZ(CLHEP::pi/1.);
 new G4PVPlacement(rotMatrix2, 
// new G4PVPlacement(0, 
                   G4ThreeVector(), 
                   fContainer_logic, 
                   "PhantomPhysical",
...
\********************* end drawing ************************/
 ...
 double polyprop_R11min = polyprop_R11max - polyprop_sizeZ1;
 ...
 double polyprop_R2min = polyprop_R2max - polyprop_sizeZ1;
...
//       0, polyprop_R2min, polyprop_L2/2.0, Phi1, Phi2);     //its size
       0, polyprop_R2min, polyprop_L2/2.0, 0, CLHEP::twopi);     //its size
...
//       0, alum_R3min, alum_L3/2.0, Phi1, Phi2);     //its size
       0, alum_R3min, alum_L3/2.0, 0, CLHEP::twopi);     //its size
 ...
 G4Tubs* solidPolyprop71 =
    new G4Tubs("Polyprop71",                       //its name
       polyprop_R7min, polyprop_R7max, alum_L3/2.0, Phi1, Phi2);     //its size
 double sleep_l = 2.2*m;
 double sleep_w = 0.8*m;
//  double thet = asin(0.5*m/polyprop_R7min);
 double thet = sleep_l/(polyprop_R7max-polyprop_sizeZ)/2.0;
 cout<<"!!!!! thet "<<thet*180./CLHEP::pi<<" "<<CLHEP::pi<<" "<<thet<<endl;
  G4Tubs* solidPolyprop72 =
    new G4Tubs("Polyprop72",                       //its name
       polyprop_R7min-4.5*cm, polyprop_R7max-polyprop_sizeZ, (sleep_w)/2.0, CLHEP::pi-thet, 2.*thet);     //its size
 G4ThreeVector rotAxis(0.0, 0.0, 1.0 );
 G4RotationMatrix *rotMatrix1 = new G4RotationMatrix(rotAxis, -CLHEP::pi/4.);
 G4SubtractionSolid* solidPolyprop7  = new G4SubtractionSolid("Polyprop7", solidPolyprop71, solidPolyprop72, nullptr,
                    G4ThreeVector(0.,0.,0.));

// G4VSolid* solidPolyprop7  = new G4SubtractionSolid("Polyprop7", solidPolyprop71, solidPolyprop72, 0,
//                    G4ThreeVector(0.,0.,0.));
//  G4SubtractionSolid* solidPolyprop7  = new G4SubtractionSolid("Polyprop7", solidPolyprop71, solidPolyprop72 );
...
//       polyprop_RCmin, polyprop_RCmax, CLHEP::pi*1./2., CLHEP::pi*2.8/2., CLHEP::pi-my_phi_prop, my_phi_prop);     //its size
 ...
 double polyprop_R41min = polyprop_R41max - polyprop_sizeZ1;
 double polyprop_R42max = steel_R42min;
 double polyprop_R42min = polyprop_R42max - polyprop_sizeZ1*cos(CLHEP::pi/4.);
 ...
 double polyprop_R5min = polyprop_R5max - polyprop_sizeZ1;
 ...
 double polyprop_R51min = polyprop_R51max - polyprop_sizeZ1*cos(CLHEP::pi/4.);
 double polyprop_R52max = polyprop_R5min*cos(CLHEP::pi/4.);
 double polyprop_R52min = polyprop_R5min*cos(CLHEP::pi/4.);
 double polyprop_L51 = polyprop_sizeZ1*cos(CLHEP::pi/4.);
...
//    fantom tube
  ...
  G4Sphere* solidSphere =
    new G4Sphere("Sphere",                       //its name
       R2, R1, 0, CLHEP::twopi, 0, pi);     //its size
  G4LogicalVolume* logic_sphere = new G4LogicalVolume(solidSphere,        //its solid
                                       iss_mat,   //its material
                                       "Sphere");       //its name
  G4VPhysicalVolume* physic_sphere = new G4PVPlacement(0,  //rotation
//                        G4ThreeVector(0.0, 0.0, (alum_L1/2.+alum_L2/2.)),
//                        G4ThreeVector(-polyprop_Rmin+R1+1*mm, 0.0, 0.0),
                        G4ThreeVector(0.0, 0.0, 0.0),
                        logic_sphere,                            //its logical volume
                        "Sphere",                                       //its name
//                        logicWorld,                           //its mother  volume
                        logicAir2,                           //its mother  volume
                        false,                                  //no boolean operation
                        0);                                     //copy number
  physic_sphere->CheckOverlaps();
  double layer = 10.*mm;
  int nlayers = (R1-R2)/layer;
  cout<<"nlayers "<<nlayers<<endl;
  const G4String pref = "Layer";
  G4String temp_str;
  G4LogicalVolume* logic_vol;
  G4VPhysicalVolume* physic_vol;
  G4Sphere* solid_sphere;

  for (int i = 0; i < nlayers; i++)
  {
   double r1=R1-i*layer;
   double r2=r1-layer;
   cout<<"i r1 r2 "<<i<<" "<<r1<<" "<<r2<<endl;
   temp_str = pref + std::to_string(i);
   cout<<temp_str<<endl;
   solid_sphere =
    new G4Sphere("sphere",                       //its name
       r2, r1, 0, CLHEP::twopi, 0, pi);     //its size
   logic_vol = new G4LogicalVolume(solid_sphere,        //its solid
                                       water_mat,   //its material
                                       "sphere");       //its name
   physic_vol = new G4PVPlacement(0,  //rotation
                        G4ThreeVector(0.0, 0.0, 0.0),
                        logic_vol,                            //its logical volume
                        temp_str,                                       //its name
//                        logicWorld,                           //its mother  volume
                        logic_sphere,                           //its mother  volume
                        false,                                  //no boolean operation
                        0);                                     //copy number
   physic_vol->CheckOverlaps();
  }
\****************************************************/



 auto* containerSolid = new G4Box("phantomBox", fPhantomSize.x()/2 + 4.*cm,
					           fPhantomSize.y()/2 + 4.*cm,
						   fPhantomSize.z()/2 + 4.*cm);
...
/*
 G4RotationMatrix *rotMatrix3 = new G4RotationMatrix();
 rotMatrix3->rotateX(CLHEP::pi/2.);
 rotMatrix3->rotateZ(-CLHEP::pi/2.);
// rotMatrix3->rotateY(-CLHEP::pi/2.);
 new G4PVPlacement(rotMatrix3, 
                   G4ThreeVector(-(polyprop_R7min-30.5*cm), 0, 0), 
//                   G4ThreeVector(), 
                   fContainer_logic, 
                   "PhantomPhysical",
//	           logicWorld, 
