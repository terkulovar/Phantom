...
// char* name2;
 ...
 Double_t stepl_t, edep_t, edep_t1, edep_t2, edep_t3, edep_t4, edep_t5;
 Double_t mystep, mystep_t;
...
//   cout<<"i dose ziv z "<<i<<" "<<dose<<" "<<edep<<" "<<ziv<<" "<<zpos<<" "<<evId<<" "<<name<<" "<<name2<<endl;
  ...
  if ( evId != iold10 && strcmp(name,"Test" ) == 0 )
  {
//   cout<<"new event "<<evId<<endl;
   
//   cout<<"!!!!!!!!!!!!!!!!!!!!!!!!!!!!stepl mystep "<<stepl_t<<" "<<mystep_t<<" "<<edep_t<<endl;
   printf("edep_t %.3f %.3f %.3f %.3f %.3f %.3f \n", edep_t, edep_t1, edep_t2, (edep_t1+edep_t2), edep_t5, (edep_t-edep_t5));
   iold10 = evId;
   stepl_t = 0;
   mystep_t = 0;
   edep_t = 0;
  ...
  { 
   stepl_t += stepl; 
   mystep_t += mystep; 
   if ( strcmp(name2,"C12" ) == 0 ) edep_t1 += edep;
   if ( strcmp(name2,"e-" ) == 0 ) edep_t2 += edep;
   if ( strcmp(name2,"proton" ) == 0 ) edep_t3 += edep;
   if ( strcmp(name2,"deutron" ) == 0 ) edep_t4 += edep;
   edep_t += edep;
   if ( strcmp(name2,"e-" ) == 0 && en < 0.005 ) edep_t5 += edep;;
   
//   cout<<"step step_t "<<stepl<<" "<<stepl_t<<" "<<edep_t<<endl; 
  }
