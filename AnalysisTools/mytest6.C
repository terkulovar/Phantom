...
// Double_t square = 2.0*pi*150.*(1200.0+150.0)/2.0;
...
// Double_t spenvis_trapped_protons_2010_368km_52degree_min_1Mev  = 3.03237e+06;
 ...
 Double_t norm = spectrum_trapped_protons_min;
...
//   if ( i1 == 0 ) { hh->Fill(
  ...
  if ( evId != iold1 && strcmp(name,"LiF" ) == 0 )
  {
   nevents1++;
   iold1 = evId;
  }
...
// out = new ofstream("spectrum_trapped_protons_min_composite_water_sphere_R_1m.txt", ios::app);
 out = new ofstream("spectrum_trapped_protons_min_4cm_composite_plane_5mm_polyprop.txt", ios::app);
 *out<<d1<<" "<<z1<<" "<<ed1<<" "<<d2<<" "<<z2<<" "<<ed2<<" "<<d3<<" "<<z3<<" "<<ed3<<" "<<d4<<" "<<z4
 <<" "<<ed4<<" "<<d5<<" "<<z5<<" "<<ed5<<" "<<d6<<" "<<z6<<" "<<ed6<<" "<<nevents<<" "<<nevents1<<endl;
// *out<<d11*norm*square*days<<" "<<z11*norm*square*days<<endl;
 out->close();
 delete out;
// cout<<"tt "<<tt<<endl;
}
